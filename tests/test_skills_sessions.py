...
def test_empty_catalog_is_safe(tmp_path):
    ...
    assert result["error"].startswith("未知技能")
...
def test_live_load_skill_semantics(manager):
    ...
    assert refused["error"].startswith("未知技能")
    ...
    assert gone["error"].startswith("未知技能")
...
def test_disable_countermand_for_loaded_skills(manager):
    ...
    assert "已被用户禁用" not in engine.context_provider()

    manager.skill_store.set_enabled("used-one", False)
    ctx = engine.context_provider()
    assert '技能「used-one」已被用户禁用' in ctx
    ...
    assert "已被用户禁用" not in engine.context_provider()  # self-healing

    manager.delete_skill("used-one")  # delete ≡ disable for the countermand too
    assert '技能「used-one」已被用户禁用' in engine.context_provider()
