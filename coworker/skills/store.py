...
def validate_name(name: str) -> str:
    ...
    if len(name) > _MAX_NAME:
        raise ValueError(f"技能名称过长（上限 {_MAX_NAME} 个字符）。")
    if ".." in name or "/" in name or "\\" in name or not _NAME_RE.match(name):
        raise ValueError(
            "技能名称只能包含字母、数字、点、连字符和下划线。"
        )
    return name
...
class SkillStore:
    ...
    def _base(self, scope: str, workspace: Optional[str | Path]) -> Path:
        ...
        if scope == PROJECT_SCOPE:
            if not workspace:
                raise ValueError("项目级技能需要指定工作区。")
            ws = Path(workspace).expanduser()
            if not ws.is_dir():
                raise ValueError(f"未知的工作区：{workspace}")
    ...
    def stage_upload(self, data: bytes, filename: str = "") -> dict[str, Any]:
        ...
        if not md_entries or len(roots) != 1:
            raise ValueError("归档文件必须恰好包含一个技能（一个 SKILL.md）。")
        root = roots.pop()
    ...
    def _stage_single_md(self, data: bytes, filename: str) -> dict[str, Any]:
        ...
        if filename.lower().endswith((".zip", ".skill")):
            raise ValueError("不是有效的 .zip 归档文件。")
        try:
            text = data.decode("utf-8")
        except UnicodeDecodeError:
            raise ValueError("不是有效的技能文件——请上传 .zip 或 SKILL.md。")
        token = uuid.uuid4().hex
        staged = self._staging_dir / token
        staged.mkdir(parents=True, exist_ok=True)
        ...
        if skill.name == token:  # no frontmatter name → parser fell back to the folder
            shutil.rmtree(staged, ignore_errors=True)
            raise ValueError(
                "该 .md 文件需要包含 YAML frontmatter，至少要有技能名称。"
            )
        try:
    ...
    *,
    ...
    def save_skill(
        ...
        if not (description or "").strip():
            return {"error": "需要一行描述——它会成为技能的菜单条目。"}
        if not (instructions or "").strip():
            return {"error": "技能说明不能为空。"}
        ...
        for raw in files or []:
            ...
            if not p.is_absolute():
                if not dirs:
                    return {"error": f"文件不在本次会话的文件夹中：{raw}"}
                p = dirs[0] / p
            try:
                rp = p.resolve()
            except OSError:
                return {"error": f"无法读取文件：{raw}"}
            if not rp.is_file():
                return {"error": f"不是文件：{raw}"}
        ...
        return {
            ...
            "note": (
                "已保存到用户的技能库——从现在起可在所有会话中使用。"
                "用一句话确认即可。要浏览已安装的文件，请引导用户"
                "前往设置 > 技能（文件数标签可打开文件夹）——不要"
                "将工作区中的构建文件夹作为附件链接；文件夹无法在那里打开。"
            ),
        }
