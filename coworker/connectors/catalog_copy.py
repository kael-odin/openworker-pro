...
ABOUT: dict[str, str] = {
    ...
    "wechat_ilink": "把发给已扫码连接账号的个人微信私聊交给 agent 处理，并把文本回复发回同一联系人。"
    "图片、文件和视频只转成占位文字，语音优先使用服务返回的转写。",
...
ACCESS: dict[str, list[str]] = {
    ...
    "wechat_ilink": [
        "只读取发给该连接账号的个人微信私聊，不读取其他聊天或群聊。",
        "以连接账号向最近有运行期会话上下文的联系人发送文本回复。",
        "图片、文件和视频仅显示占位文字，不下载真实附件；语音仅使用服务返回的转写。",
        "机器人凭据仅存本机；iLink 并非公开稳定 SDK，服务端变化可能要求重新扫码。",
    ],
    "wecom": [
