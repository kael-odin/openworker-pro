...
export function McpRow({
  ...
  const loadTools = async () => {
    ...
    else setToolErr(res.error || t("mtab.failed_connect"));
  };

  return (
    <div className={CARD + " p-3.5"}>
      <div className="flex items-center gap-3">
        <Toggle checked={server.enabled} onChange={onToggle} title={t("mtab.enable_server")} />
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-medium">{server.name}</div>
          <div className="text-[11.5px] text-faint">
            {server.transport} · {authorizing ? t("mtab.status_signing_in") : server.status.replace("_", " ")}
            {server.tool_count != null ? t("mtab.tools_n", { n: server.tool_count }) : ""}
            {server.requires_approval ? t("mtab.asks") : ""}
            {isOauth ? t("mtab.oauth") : ""}
          </div>
        </div>
        {isOauth &&
          (server.status === "needs_auth" ? (
            ...
            <span className="text-[12px] text-muted shrink-0">{t("mtab.waiting_browser")}</span>
        ...
        <button
          ...
          disabled={busy}
        >
          {busy ? "…" : tools ? t("mtab.hide_tools") : t("mtab.tools")}
        </button>
      ...
      {tools && (
        <div className="mt-2.5 pt-2.5 border-t border-line flex flex-wrap gap-1.5">
          {tools.length === 0 && <div className="text-[12px] text-faint">{t("mtab.no_tools")}</div>}
          {tools.map((tool) => (
            <span
              key={tool.name}
...
function AddForm({
  ...
  const save = async () => {
    ...
    try {
      ...
      onError(t("mtab.invalid_json") + e.message);
      return;
    }
  ...
  return (
    <div className="space-y-2">
      <div className="text-[12.5px] text-muted">{t("mtab.paste_json")}</div>
      <textarea
        value={text}
    ...
    <div className="border-t border-line px-3.5 py-3 space-y-3">
      {c.mcp && !manualOnly && (
        ...
        <div className="space-y-2" data-testid="mcp-connect">
          ...
          {c.fields.length > 0 && (
            <div className="text-[11.5px] text-faint">{t("mtab.or_manual")}</div>
          )}
        </div>
      )}
      {c.managed && !c.mcp && !manualOnly && (
        <div className="space-y-2" data-testid="managed-connect">
          ...
          {!c.managed_paused && cloud?.signed_in && (
            <div className="text-[11.5px] text-faint">{t("mtab.manual_or")}</div>
          )}
        </div>
      )}
